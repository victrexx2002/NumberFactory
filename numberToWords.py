...
def numberToWords(number_input):
    ...
    def fractions(number):
        ...
        for x in number_string:
            result = ' '.join str( result , words[ones_index + int(number_string[x])])
        return result
    ...
    '''
    if type(float(number_input)) != 'number' :
        txts = " {} is not a number"
        return txts.format(number_input)
